...
namespace FrameLog.Logging
{
        ...
        where TChangeSet : IChangeSet<TPrincipal>
    {
        ...
        public ChangeLogger(DbContext context, 
                ...
                if (filter.ShouldLog(entry.Entity.GetType(), propertyName))
                {
                    ...
                    foreach (var valuePair in valuePairs)
                    {
                        recorder.Record(entity,
                            () => DataContextHelper.GetReferenceForObject(entity),
        ...
        private void logForeignKeyChange(ObjectStateEntry entry, AssociationEndMember localEnd, AssociationEndMember foreignEnd)
        {
            ...
            object entity = DataContextHelper.GetObjectByKey(context, key);
            ...
            recorder.Record(entity, () => DataContextHelper.GetReferenceForObject(entity), property.Name, value);
        }
        ...
        private Func<object> manyToManyValue(object entity, string propertyName)
        {
            return () =>
            {
                ...
                IEnumerable<string> current = ((IEnumerable<object>)value)
                    .Select(e => DataContextHelper.GetReferenceForObject(e))
                    .Distinct()
        ...
        {
            var entity = DataContextHelper.GetObjectByKey(context, key);
            return DataContextHelper.GetReferenceForObject(entity);
        }
